...
const serverStateSlice = createSlice({
  ...
  reducers: {
    startServer: (state) => {
      ...
      state.error = null;
      console.log('[ServeSlice] Server state changed: LOADING');
    },
    serverStarted: (state) => {
      state.status = SERVER_STATES.RUNNING;
      state.startTime = Date.now();
      console.log('[ServeSlice] Server state changed: RUNNING');
    },
    serverFailed: (state, action) => {
      state.status = SERVER_STATES.IDLE;
      state.error = action.payload;
      console.log('[ServeSlice] Server state changed: IDLE (FAILED)', action.payload);
    },
    stopServer: (state) => {
      state.status = SERVER_STATES.IDLE;
      state.startTime = null;
      console.log('[ServeSlice] Server state changed: IDLE (STOPPED)');
    }
  }
});
